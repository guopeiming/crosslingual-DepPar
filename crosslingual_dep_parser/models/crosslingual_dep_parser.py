...
@Model.register("crosslingual_dep_parser")
class CrosslingualDependencyParser(Model):
    ...
    def __init__(
        ...
        # archive_overrides: Union[str, Dict[str, Any]] = "",
        ...
        **kwargs,
        ...
        #     self.load_archive_weight(archive_file, archive_overrides)
    ...
    def forward(
        ...
        # shape (batch_size, sequence_length, arc_representation_dim/tag_representation_dim)
        head_arc_representation, child_arc_representation, head_tag_representation, child_tag_representation, _ = \
            self.encode_text(tokens, mask, postags, language)
    ...
    def encode_token_representations(
        self,
        tokens: TextFieldTensors,
        postags: torch.LongTensor,
        language: torch.LongTensor,
    ) -> torch.Tensor:
        embedded_text_input = self.text_field_embedder(tokens, pgn_ids=language)
    ...
    def encode_text(
        ...
        embedded_text_input = self.encode_token_representations(tokens, postags, language)
    ...
    def _construct_loss(
        ...
        # mask the loss of positions with low confidence.
    ...
    def _get_mask_for_eval(
        ...
        """
        new_mask = mask #& (confidences >= self.threshold)
        return new_mask
    ...
    # def load_archive_weight(
    #     self,
    #     archive_file: str,
    #     archive_overrides: Union[str, Dict[str, Any]] = ""
    # ) -> None:
    #     model = load_archive(archive_file, cuda_device=-1, overrides=archive_overrides).model

    #     only_dropout_modules = [
    #         'lstm_encoder', 'postags_embedding',
    #         'head_arc_feedforward', 'child_arc_feedforward',
    #         'head_tag_feedforward', 'child_tag_feedforward',
    #         'arc_attention', 'tag_bilinear',
    #     ]
    #     dropout_p = 0.2
    #     with torch.no_grad():
    #         for module_name in only_dropout_modules:
    #             module = dict(getattr(model, module_name).named_parameters())
    #             module_self = dict(getattr(self, module_name).named_parameters())
    #             for name in module:
    #                 module_self[name].copy_(F.dropout(module[name], dropout_p, True, True))

    #         module = dict(model.text_field_embedder.token_embedder_tokens.pgn_groups.named_parameters())
    #         module_self = dict(self.text_field_embedder.token_embedder_tokens.pgn_groups.named_parameters())
    #         for name in module:
    #             module_self[name].copy_(F.dropout(module[name], dropout_p, True, True))

    #         module = dict(model.text_field_embedder.token_embedder_token_characters._encoder.named_parameters())
    #         module_self = dict(self.text_field_embedder.token_embedder_token_characters._encoder.named_parameters())
    #         for name in module:
    #             module_self[name].copy_(F.dropout(module[name], dropout_p, True, True))

    #         for i in range(12):
    #             module = model.text_field_embedder.token_embedder_tokens._matched_embedder.transformer_model.encoder.layer[i].output.base.LayerNorm
    #             module_self = self.text_field_embedder.token_embedder_tokens._matched_embedder.transformer_model.encoder.layer[i].output.base.LayerNorm
    #             module_self.weight.copy_(F.dropout(module.weight, dropout_p, True, True))
    #             module_self.bias.copy_(F.dropout(module.bias, dropout_p, True, True))

    #             module = model.text_field_embedder.token_embedder_tokens._matched_embedder.transformer_model.encoder.layer[i].attention.output.base.LayerNorm
    #             module_self = self.text_field_embedder.token_embedder_tokens._matched_embedder.transformer_model.encoder.layer[i].attention.output.base.LayerNorm
    #             module_self.weight.copy_(F.dropout(module.weight, dropout_p, True, True))
    #             module_self.bias.copy_(F.dropout(module.bias, dropout_p, True, True))

    #         module = model.text_field_embedder.token_embedder_token_characters._embedding._module
    #         module_self = self.text_field_embedder.token_embedder_token_characters._embedding._module
    #         module_self.weight[:module.num_embeddings, :] = F.dropout(module.weight, dropout_p, True, True)

    #         module = model.text_field_embedder.token_embedder_tokens.embedding
    #         module_self = self.text_field_embedder.token_embedder_tokens.embedding
    #         if module_self.num_embeddings != module.num_embeddings+1:
    #             raise ConfigurationError('pgn embedding num error')
    #         module_self.weight[:module.num_embeddings, :] = F.dropout(module.weight, dropout_p, True, True)
    #         module_self.weight[module.num_embeddings, :] = F.dropout(module.weight[module.num_embeddings-1, :], 0.5, True, True)
